...
export default function LedgerRow({ run, expanded, onExpandToggle, onResume }) {
  ...
  return (
    <motion.div layout className="border-b border-edge/60 last:border-0">
      ...
      >
        <span className="flex-1 min-w-0 text-sm text-white/80 font-body truncate">
          {truncate(run.brief_text, 90)}
        </span>
        <span className="w-28 shrink-0"><StatusBadge status={run.status} /></span>
        <span className="w-28 shrink-0 text-right text-sm text-white font-semibold font-body">
          {totalCost != null ? `₹${totalCost.toLocaleString("en-IN")}` : "—"}
        </span>
        <span className="w-32 shrink-0 text-right text-xs text-white/30 font-body tabular-nums">
          {formatTimestamp(run.created_at)}
        </span>
        {isLive ? (
          <span className="w-4 shrink-0 text-right text-white/20">↗</span>
        ) : (
          <motion.span
            className="w-4 shrink-0 text-right text-white/20"
      ...
      <AnimatePresence initial={false}>
        {expanded && !isLive && (
          ...
          >
            <div className="pb-5">
              {!detail ? (
                <p className="text-xs text-white/30 font-body py-3">Loading details…</p>
              ) : decision?.chosen?.mode === "infeasible" ? (
                <div className="rounded-2xl border border-rose-500/30 bg-rose-500/5 p-4">
                  <p className="text-sm text-rose-300 font-body">Couldn't find a fit</p>
                  <p className="mt-1 text-xs text-white/50 font-body">{decision.why_rejected}</p>
                </div>
              ) : decision ? (
                <DecisionPanel decision={decision} embedded />
              ) : (
                <p className="text-xs text-white/40 font-body py-3">No decision recorded for this run.</p>
              )}
            </div>
          </motion.div>
        )}
